...
block_sibportfolio_require_login_system('/blocks/sibportfolio/handlers/forms/rejectclaim.php', array('claimid' => $claimId, 'returnurl' => $returnUrl));

$PAGE->set_title(get_string('key125', 'block_sibportfolio'));
...
if ($claim = block_sibportfolio_check_access($claimId)) {
    ...
    if ($mform->is_cancelled()) {
        ...
        $event = \block_sibportfolio\event\claim_abandoned::create(array(
            'objectid' 		=> $claimId,
        ...
        $PAGE->navbar->add(get_string('key125', 'block_sibportfolio'), $PAGE->url);		
