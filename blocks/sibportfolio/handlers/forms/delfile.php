...
block_sibportfolio_require_login_system('/blocks/sibportfolio/handlers/forms/delfile.php', array('claimid' => $claimId, 'returnurl' => $returnUrl));

$PAGE->set_title(get_string('key114', 'block_sibportfolio'));
...
if ($claim = block_sibportfolio_check_access($claimId)) {
    ...
    $customData = array('accepted' => is_siteadmin() || block_sibportfolio_is_curator($userId) || block_sibportfolio_i_am_curator($userId));
    $mform = new delFileForm(null, $customData);
    ...
    if ($mform->is_cancelled()) {
        ...
        if ($action) {
            ...
            $logData = array(
                ...
                'comment' 	  => get_string_manager()->get_string('key116', 'block_sibportfolio', null, $CFG->lang),
                'itemid' 	  => $claimId
            );
        ...
        $request->category = $claim->category;
        $request->comment = isset($fromform->cause) ? trim($fromform->cause) : null;
        block_sibportfolio_claims::register_claim($request);
        
        if ($action) {
            $event = \block_sibportfolio\event\file_deleted::create(array(
                'objectid' => $claimId,
                'relateduserid' => $userId
            ));
            $event->trigger();
        } else {
            $event = \block_sibportfolio\event\claim_delete_created::create(array(
                'objectid' => $claimId
            ));
            $event->trigger();
        }	
        ...
        $header = $action ? get_string('key114', 'block_sibportfolio') : get_string('key115', 'block_sibportfolio');
...
function block_sibportfolio_check_access($claimId) {
    ...
    if (!$claim) {
        ...
        print_error('key118', 'block_sibportfolio');
        return false;
